  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (token) {
        setShowSidebar(true);
        try {
          const response = await axios.get('https://librarysystem-backend.onrender.com/api/v1/recommend', {
            headers: {
    ...
    <aside className={cx('sidebar')}>
      ...
      <div className={cx('book-list')}>
        {currentBooks.map(book => (
          <Link to={`/book/${convertToSlug(book.title)}`} key={book._id} className={cx('book-item')}>
